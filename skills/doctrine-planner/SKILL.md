...
3. Create a JSON scratch plan outside the target repository. Include the frozen exact-commit `doctrine_ref`, `scope`, `allowed_paths`, `validation`, `stop_conditions`, `unknowns`, `finished_state`, `acceptance_condition`, `invariants`, `implementation_options`, `selected_implementation`, `complexity_justification`, `remaining_diff`, `next_fill_order`, and `milestones`. Limit `implementation_options` to options that satisfy the same acceptance condition, invariants, and applicable target constraints. For each option, describe its mechanisms, state, dependencies, and special cases. Set `selected_implementation` to the exact selected list member. Use `complexity_justification` to name every retained complexity and the requirement, invariant, or applicable target constraint that requires it; use `none_observed` when no added complexity is retained.
...
The linter validates plan structure, implementation-selection references, ordering, and exact `doctrine_ref` shape only. It does not prove that listed options satisfy the same acceptance condition and invariants, that the selected option is actually simpler, that a complexity justification is true, or that the commit was the observed `main` head, authenticated authority, runtime firing, GitHub state, or completion. Keep source acquisition evidence internal. Cite only target-specific acquisition and firing evidence in delivered artifacts when the skeleton executes.
